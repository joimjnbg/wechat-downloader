...
- **Sync depth is staged: latest-only in v1, full history later (ticket-02 spike, 2026-09-15).** The only live discovery channel is Weread `/api/mp/cover`, which yields the latest ONE article per Account per check (identity cursor, no publish time). The full-history channel (`appmsgpublish`, 20 groups/page) is code-intact but gateway-retired with zero network; the Weread mobile list is server-side per-account gated (-2041, upstream 2026-08-27 terminal spike — do not re-drill). So v1 sync = latest-only incremental + per-URL/URL-list download; the picker, archive, dedupe, and library UX run on top of whatever the list yields. Full-history backfill is staged, not dropped. Evidence: `.scratch/wechat-downloader/history-depth-report.md`.
